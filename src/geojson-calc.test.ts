...
describe("distance", () => {
  it.each<[[gj.Coordinate, gj.Coordinate], number]>([
    ...
    [
      ...
      1861.65,
    ], // test at 90lon to ensure I didn't mix up lat and lon
    ...
    "distance(%p) = %p",
    (args: [gj.Coordinate, gj.Coordinate], expected: number) => {
...
describe("douglasPeucker", () => {
  it.each<[[gj.Coordinate[], number], gj.Coordinate[]]>([
    ...
    "douglasPeucker(%p) = %p",
    (args: [gj.Coordinate[], number], expected: gj.Coordinate[]) => {
...
describe("simplify", () => {
  it.each<[[gj.Feature, number], gj.Feature]>([
    ...
    [
      [
        {
          type: gj.FeatureType.Feature,
          properties: { name: "REI" },
          geometry: {
            type: gj.GeometryType.Point,
            coordinates: [-122.466, 47.22229],
          },
        },
        1852 * 6,
      ],
      {
        type: gj.FeatureType.Feature,
        properties: { name: "REI" },
        geometry: { type: gj.GeometryType.Point, coordinates: [-122.5, 47.2] },
      },
    ],
  ])(
    "simplify(%p) = %p",
    (args: [gj.Feature, number], expected: gj.Feature) => {
