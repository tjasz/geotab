...
export function distance(pta:gj.Coordinate, ptb:gj.Coordinate) : number {
  ...
  const a = sin_half_delta_lat * sin_half_delta_lat
          + math.dcos(ptb[1]) * math.dcos(pta[1])
...
function nearestPointGreatCircle(p0:gj.Coordinate,ls:[gj.Coordinate,gj.Coordinate]) : gj.Coordinate {
  const [p1, p2] = ls;
  ...
  const T = math.scalarProduct(math.normalize(math.crossProduct(G, F)), earthRadiusMeters);
...
export function onSegment(p0:gj.Coordinate,ls:[gj.Coordinate,gj.Coordinate]) : boolean {
...
export function distanceToLineSeg(p0:gj.Coordinate,ls:[gj.Coordinate,gj.Coordinate]) : number {
...
function distancePointToLineString(a:gj.Point, b:gj.LineString) : number {
  ...
  for (let i:number = 1; i < b.coordinates.length; i++) {
    const candidate = distanceToLineSeg(a.coordinates, [b.coordinates[i-1],b.coordinates[i]]);
    if (candidate < res) {
      res = candidate
    }
  }
...
function distancePointToMultiLineString(a:gj.Point, b:gj.MultiLineString) : number {
  let res = Infinity;
  for (let i:number = 0; i < b.coordinates.length; i++) {
    for (let j:number = 1; j < b.coordinates[i].length; j++) {
      const candidate = distanceToLineSeg(a.coordinates, [b.coordinates[i][j-1],b.coordinates[i][j]]);
      if (candidate < res) {
        res = candidate
      }
    }
...
type featureDistanceFunction = {(a:gj.Geometry|gj.GeometryCollection,b:gj.Geometry|gj.GeometryCollection) : number}
// TODO define all featureDistanceFunctions
const featureDistanceFunctions : [gj.GeometryType, gj.GeometryType, featureDistanceFunction][] = [
  [gj.GeometryType.Point,gj.GeometryType.Point,(a,b)=>distancePointToPoint(a as gj.Point,b as gj.Point)],
  [gj.GeometryType.Point,gj.GeometryType.MultiPoint,(a,b)=>distancePointToMultiPoint(a as gj.Point, b as gj.MultiPoint)],
  [gj.GeometryType.Point,gj.GeometryType.LineString,(a,b)=>distancePointToLineString(a as gj.Point, b as gj.LineString)],
  [gj.GeometryType.Point,gj.GeometryType.LineString,(a,b)=>distancePointToMultiLineString(a as gj.Point, b as gj.MultiLineString)],
];
export function distanceBetweenFeatures(a:gj.Feature, b:gj.Feature) : number {
  const functions = featureDistanceFunctions.filter(([t1,t2,f]) => t1 === a.geometry.type && t2 === b.geometry.type);
  if (functions.length === 0) {
    throw Error(`Distance function not defined for feature geometries of types: ${a.geometry.type}, ${b.geometry.type}.`);
  }
...
function simplifyCoordinate(c:gj.Coordinate, precisionMeters:number) : gj.Coordinate {
...
function simplifyPoint(p:gj.Point, precisionMeters:number) : gj.Point {
  ...
  return {...rest, coordinates: simplifyCoordinate(coordinates, precisionMeters)};
}
function simplifyMultiPoint(p:gj.MultiPoint, precisionMeters:number) : gj.MultiPoint {
  const {coordinates, ...rest} = p;
  return {
    ...rest,
...
export function simplify(f:gj.Feature, precisionMeters:number) : gj.Feature {
  switch (f.geometry.type) {
    case gj.GeometryType.Point:
      return {...f, geometry: simplifyPoint(f.geometry as gj.Point, precisionMeters)};
    case gj.GeometryType.MultiPoint:
      return {...f, geometry: simplifyMultiPoint(f.geometry as gj.MultiPoint, precisionMeters)};
    case gj.GeometryType.LineString:
      return {...f, geometry: simplifyLineString(f.geometry as gj.LineString, precisionMeters)};
    case gj.GeometryType.MultiLineString:
      return {...f, geometry: simplifyMultiLineString(f.geometry as gj.MultiLineString, precisionMeters)};
    case gj.GeometryType.Polygon:
      return {...f, geometry: simplifyPolygon(f.geometry as gj.Polygon, precisionMeters)};
    case gj.GeometryType.MultiPolygon:
      return {...f, geometry: simplifyMultiPolygon(f.geometry as gj.MultiPolygon, precisionMeters)};
