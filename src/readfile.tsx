...
import { csvToGeoJSON } from "./file-formats/csvToGeojson";

export function attachProgress(
  proms: Promise<any>[],
  progress_cb: { (percentDone: number): void },
): void {
  let d = 0;
  progress_cb(0);
    ...
    readFileAsText(file).then((text) => {
      ...
      if (text[0] === "{") {
        ...
        try {
          ...
          reject(
            `Could not parse ${file.name} as GeoJSON: ${error instanceof Error ? error.message : JSON.stringify(error)}.`,
          );
        }
      } else if (text[0] === "<") {
        // GPX
        try {
          ...
          reject(
            `Could not parse ${file.name} as GPX: ${error instanceof Error ? error.message : JSON.stringify(error)}.`,
          );
        }
          ...
          readFileAsArrayBuffer(file).then((arraybuffer) => {
            try {
              ...
              fit.parse(arraybuffer, (error, data) => {
                if (error) {
                  reject(
                    `Could not parse ${file.name} as .FIT: ${error instanceof Error ? error.message : JSON.stringify(error)}.`,
                  );
                } else {
        ...
        try {
          ...
          reject(
            `Could not parse ${file.name} as CSV: ${error instanceof Error ? error.message : JSON.stringify(error)}.`,
          );
        }
      }
    });
...
function getGoogleFile(file) {
  switch (file.mimeType) {
    ...
    default:
      return gapi.client.drive.files.get({ fileId: file.id, alt: "media" });
  }
}
...
function fitToGeoJSON(fit): GeoJson.FeatureCollection {
  ...
  for (let i = 0; i < fit.records.length; i++) {
    ...
    if (record.altitude) {
      if (
        track.properties.minElevation === undefined ||
        record.altitude < track.properties.minElevation
      ) {
        track.properties.minElevation = record.altitude;
      }
      if (
        track.properties.maxElevation === undefined ||
        record.altitude > track.properties.maxElevation
      ) {
      ...
      if (lastElev) {
        ...
        if (change > 0) {
          track.properties.gain = track.properties.gain
            ? track.properties.gain + change
            : change;
        } else {
          track.properties.loss = track.properties.loss
            ? track.properties.loss - change
            : -change;
        }
      }
