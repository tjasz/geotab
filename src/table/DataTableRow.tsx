...
type TableRowProps = {
  ...
  onClick: (
    event: React.MouseEvent<HTMLTableRowElement, MouseEvent> | undefined,
    f: Feature,
  ) => void;
...
export default function DataTableRow(props: TableRowProps) {
  ...
  const [className, setClassName] = useState(
    props.feature.properties["geotab:selectionStatus"] ?? "inactive",
  );
  if (context !== null) {
    context.setFeatureListener("table", props.feature.id, (f) => {
      setClassName(f.properties["geotab:selectionStatus"]);
      props.onClick(undefined, f);
    });
  }
  ...
  return (
    <TableRow
      ...
      onClick={(e) => {
        props.feature.properties["geotab:selectionStatus"] = toggleActive(
          props.feature.properties["geotab:selectionStatus"],
        );
        props.onClick(e, props.feature);
        setClassName(props.feature.properties["geotab:selectionStatus"]);
        const mapListener =
          context?.featureListeners.map[props.feature.id] ??
          context?.featureListeners.map["default"];
        if (mapListener !== undefined) {
          mapListener(props.feature);
        }
      }}
      onMouseOver={(e) => {
        props.feature.properties["geotab:selectionStatus"] = addHover(
          props.feature.properties["geotab:selectionStatus"],
        );
        setClassName(props.feature.properties["geotab:selectionStatus"]);
        const mapListener =
      ...
      onMouseOut={(e) => {
        props.feature.properties["geotab:selectionStatus"] = removeHover(
          props.feature.properties["geotab:selectionStatus"],
        );
        setClassName(props.feature.properties["geotab:selectionStatus"]);
        const mapListener =
