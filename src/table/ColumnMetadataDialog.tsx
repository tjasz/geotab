...
export function ColumnMetadataDialog(props: ColumnMetadataDialogProps) {
  return props.column === undefined ? null : (
    ...
    <React.Fragment>
      ...
      {(stats as StringStats).counts !== undefined && (
        <React.Fragment>
          ...
          <table>
            <tbody>
              {(stats as StringStats).counts.map(([value, count]) => (
                <tr key={value}>
                  <th>{value}</th>
                  <td>{count}</td>
                </tr>
...
function histogramFromStats(
  stats: GenericStats & NumericStats,
  data: number[],
): JSX.Element | false {
  return (
    stats.defined !== undefined &&
    stats.defined > 0 &&
...
type Stats =
  | GenericStats
  | (GenericStats & StringStats)
  | (GenericStats & NumericStats);
...
function getStringStats(data: string[]): StringStats {
  ...
  const sortedCounts = Object.entries(counts).sort(
    ([astr, acount], [bstr, bcount]) => bcount - acount,
  );
  if (sortedCounts.length > maxCounts) {
    sortedCounts[maxCounts - 1] = [
      "OTHER",
      sortedCounts
        .slice(maxCounts - 1)
        .reduce((s, [str, count]) => s + count, 0),
    ];
  }
  return {
    minimum,
