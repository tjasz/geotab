...
export function mapMin<T>(a:T[], f:numberTransform<T>) : number {
  return a.reduce((m,v) => {const fv = f(v); return fv < m ? fv : m}, Infinity);
}

export function dotProduct(a:number[], b:number[]) {
  if (a.length !== b.length) {
    throw Error(`Cannot take dot product of different-length vectors: ${a}, ${b}.`)
  }
  return sum(a.map((ac,i) => ac*b[i]));
}

export function crossProduct(a:[number,number,number], b:[number,number,number]) : [number,number,number] {
  const [a1,a2,a3] = a;
  const [b1,b2,b3] = b;
  return [
    a2*b3 - a3*b2,
