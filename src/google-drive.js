...
function GoogleSession(props) {
  ...
  const [isFetchingGoogleDriveFile, setIsFetchingGoogleDriveFile] =
    useState(false);
      ...
      gapi.load("client", () => {
        initClient(() => {
          ...
          gapi.client
            ...
            .then((response) =>
              updateSigninStatus(response.result.emailAddresses[0].value),
            );
        });
      });
    },
  ...
  const saveNewFile = (filename) => {
    ...
    upload({ name: filename }, (uploadResult) => {
      setOpenFile(uploadResult);
      setIsSavingGoogleDriveFile(false);
    });
  };
...
function UserButton(props) {
  return props.isLoading ? (
    <button onClick={props.handleSignOutClick} id="signinStatus">
      <CircularProgress size={36} />
      <p>Signing in...</p>
    </button>
  ) : props.signedInUser ? (
    ...
    <button onClick={props.handleSignInClick} id="signinStatus">
